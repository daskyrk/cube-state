...
import { Effects, EnhanceReducers, Opt, Reducers, EnhanceReducerFn, StateSelector, Updater, Plugin, HookMap, EnhanceEffects, InitOpt, StoreMap, calledFn } from './index.d';
import { useMount, isPromise } from './util';
...
// }
function createStore<S, R extends EnhanceReducers<S>, E extends EnhanceEffects<S>>(opt: Opt<S, R, E>) {
      ...
      effects[fnName] = async function <A, B> (payload: A, extra?: B) {
        ...
        produce<any, any>(payload, (pay: any) => {
          for (const beforeEffect of hookMap.beforeEffect as Function[]) {
            const p = beforeEffect({ storeName, effectName: fnName, payload: pay, extra, ...effectMeta });
        ...
        produce<any, any>(result, (res: any) => {
          for (const afterEffect of hookMap.afterEffect as Function[]) {
            const p = afterEffect({ storeName, effectName: fnName, payload, result: res, ...effectMeta });
            isPromise(p) && ps.push(p);
          }
        });
        await Promise.all(ps);
    ...
    Object.keys(opt.reducers).forEach((fnName) => {
      ...
      reducers[fnName] = function (...payload: any) {
        ...
        const nextState: S = produce<S, S>(storeState, (draft: S) => {
          (hookMap.beforeReducer || []).forEach(beforeReducer => beforeReducer({ storeName, reducerName: fnName, payload }));
          result = originalReducer(draft, ...payload);
          (hookMap.afterReducer || []).forEach(afterReducer => afterReducer({ storeName, reducerName: fnName, payload }));
          return result;
        });
        ...
        return result;
      } as EnhanceReducerFn<R[Extract<keyof (R extends undefined ? undefined : R), string>]>;
    });
  }
  ...
  const newStore = { name: opt.name, state: opt.state, useStore, effects, reducers, getState, updateState };
